...
function ListSpeciality() {
    ...
    const deleteDonaton = (id) => {
        const durl = `${API_URL}/${DELETEDOCTORSPECIALITY}`;
        axios.delete(durl, { data: { 'speciality_id': id } }, {
        })
            .then(response => {
                if (response.data.status === 204) {
                    success(toast.success(response.data.message))
                     history.push('/list-speciality',{state:{status:'done'}})
                }
            })
            .catch(error => {
                console.log(error)
                    ...
                    <div className='content-header'>
                        <div className='d-flex align-items-center'>
                            <div className='me-auto'>
                                <h4 className='page-title'>Speciality LIST</h4>
                                <div className='d-inline-block align-items-center'>
                                    <nav>
                                        <ol className='breadcrumb'>
                                            ...
                                            <li className='breadcrumb-item'>
                                                <button><Link to='/add-speciality'>Speciality Add
                                                </Link></button>
                                            </li>
                                        </ol>
                                    </nav>
